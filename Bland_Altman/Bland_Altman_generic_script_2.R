...
MT <- as_tibble(read.csv("/Volumes/InmanDrive4 1/Dropbox (Personal)/KPI/University_of_Dundee/Dissertation/Data/Output_files/AllResults_Data/Master_Table.csv"))
...
blandr.draw(ba$TotlogLR.x, ba$TotlogLR.y, method1name = "EFM",
            ...
            ggplot(data=tmp, mapping = aes(x=bland.stats$means, y = dif.methods)) +
              geom_point() +
              geom_smooth()
...
# Some play 
tmp <- tibble(dif.methods,bland.stats$means)
ggplot(data = ba) +
  geom_point(mapping = aes(x = TotlogLR.x, y = TotlogLR.y)) +
  geom_smooth(mapping = aes(x = TotlogLR.x, y = TotlogLR.y))
  facet_wrap(~ TotDNA.x)
